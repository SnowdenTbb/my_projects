...
def extracting_values_from_dict_into_list(list_with_key: list, dictionary: Dict[str, Any]) -> List[float]:
    list_with_values = list()

    for key in list_with_key:
...
class FairValueOfBond:
    """
    Methods, which calculating
    fair value of bond, ytm
    """

    def __init__(self, table_with_cf, dict_with_yield_term_pairs, bond_dirty_price, face_value):
        self.table_with_cf = table_with_cf
        self.dict_with_yield_term_pairs = dict_with_yield_term_pairs

        self.bond_dirty_price = bond_dirty_price
        self.face_value = face_value

        self.list_with_bond_term = list(self.table_with_cf['term'])
        self.list_with_bond_yield = extracting_values_from_dict_into_list(
            list_with_bond_term, self.dict_with_yield_term_pairs)
    ...
    def calculating_fv_of_bond(self, z_spread: float) -> float:
        ...
        kwargs = dict(table_with_cf=self.table_with_cf,
                      list_with_yield=self.list_with_bond_yield)
